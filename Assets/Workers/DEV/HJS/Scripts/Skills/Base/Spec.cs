...
public struct Spec
{
    ...
    public float Power(int level) { return ( 0 < level && level <= actValue.Length) ? actValue[level - 1].x * statModel.ElementalPowerPer : 0; }
    ...
    public float InteractionDamage(int level) { return (0 < level && level <= interactionValue.Length) ? interactionValue[level - 1].x * statModel.ElementalPowerPer : 0; }
